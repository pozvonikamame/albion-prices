...
export function readBlackMarketSnapshot(): BlackMarketSnapshot {
  try {
    if (!existsSync(CACHE_FILE)) return EMPTY_SNAPSHOT;
    const parsed = JSON.parse(readFileSync(CACHE_FILE, "utf8")) as BlackMarketSnapshot;
    if (!Array.isArray(parsed.rows)) return EMPTY_SNAPSHOT;
    return {
      rows: parsed.rows,
